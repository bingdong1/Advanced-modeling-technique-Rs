...
for(i in 1:length(corrFeature1)){
  p[[i]] <- ggplot(coverData1, aes_string(x=corrFeature1[i],y=corrFeature2[i])) +
...
rf<- randomForest(coverType~. ,coverData1[ sampleIndex,], importance.type = 1) #1=mean decrease in 
...
train3 <- train[ , colnames(train[,13,with=FALSE]):=NULL]


drop <- c("coverType") 

train1 <- coverData1[,!(names(train) %in% drop)]


bor.results <- Boruta(boruta_features, train$coverType,
                     maxRuns=200)
