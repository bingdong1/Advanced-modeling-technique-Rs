...
# Feature Importance
  ...
  #Elevation has the higest imporatance, but the second highest is differnt from the random forest importance index. 
...
table(mypredRFTrain, train$coverType)   
...
#Training Accuracy
weighted.acc(mypredANNTrain,normed$coverType)  #0.4180

#Testing Accuracy
weighted.acc(mypredANNTest,normedTest$coverType)  #0.4166
...
for (i in 1:1)
{
  for (c in 1:length(class.weights[,1]))
  {
    ...
    for (h in 2:16)
    {
      ...
      # With range
#       ann <- nnet(train$coverType ~ ., data=normed[train_ind,-3], weights=data.weights[train_ind], size=h, decay=5e-4, rang=(1/max(normed[,-c(11,12,13)])), maxit=200)
...
#best model
weighted.acc(pred1[train_ind],normed[train_ind,3])  

rm(mypredRFTrain)

# Save everything 
save(results, file=paste0("U:/Midterm//ann.results.RData")) 
save(models, file=paste0("G:/Employee Folders/Dong Bing//ann.models.RData")) 
