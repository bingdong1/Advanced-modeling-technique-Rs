...
#Normalized the data
#normed <- cbind(coverData1[,11:13], as.data.frame(lapply(coverData1[,-c(11,12,13)], function(col) { col / max(abs(col)) })))

# load("G:/Employee Folders/Dong Bing//best.ann.RData")
# 
# pred1 <- predict(best.ann, normed[,-3], type="class")
# 
# 
# #best model
# #Train Accuracy
# 
# pred_train <- pred1[train_ind]
# data_train <- normed[train_ind,]
# 
# weighted.acc(pred_train,data_train$coverType) #0.7809226   
# confusionMatrix(pred_train, data_train$coverType) #0.6553
# 
# #Test Accuracy
# 
# pred_test <- pred1[-train_ind]
# data_test <- normed[-train_ind,]
# 
# weighted.acc(pred_test,data_test$coverType) #0.7838392 
# confusionMatrix(pred_test, data_test$coverType) #0.6556
# 
# 
...
normed <- normed[ sampleIndex,]

##########################################################

## 70% of the sample size
smp_size <- floor(0.70 * nrow(normed))
...
for (i in 1:1)
{
  for (c in 1:length(class.weights[,1]))
  {
    ...
    for (h in 2:16)
    {
      ...
      ann <- nnet(train$coverType ~ ., data=normed[train_ind,-3], weights=data.weights[train_ind], size=h, decay=5e-4, maxit=200)
      pred <- predict(ann, normed[,-3], type="class")
      tacc <- weighted.acc(pred[train_ind], normed[train_ind,3])
      wacc <- weighted.acc(pred[-train_ind], normed[-train_ind,3])
#      pacc <- sum(pred[-train_ind]==normed[-train_ind,2])/length(pred[-train_ind])
      confusion <- confusionMatrix(gbm_predTest, data.test1$coverType)$overall[1]
      
      results <- rbind(results, c(h, tacc, wacc, confusion, c, 1))
...
#best model
#Train Accuracy
weighted.acc(pred1[train_ind],normed[train_ind,3]) #0.783039426   
table(pred1[train_ind],normed[train_ind,3])

#Test Accuracy
weighted.acc(pred1[-train_ind],normed[-train_ind,3]) #0.778904944  
table(pred1[-train_ind],normed[-train_ind,3])


# Save everything 
save(results, file=paste0("U:/Midterm//ann.results.RData")) 
save(models, file=paste0("U:/Midterm//ann.models.RData")) 
save(best.ann, file=paste0("U:/Midterm//best.ann.RData")) 
write.table(results, file=paste0("U:/Midterm//ann.results.csv"), sep=",") 
