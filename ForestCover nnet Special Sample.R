...
for (i in 1:1)
{
  for (c in 1:length(class.weights[,1]))
  {
    ...
    for (h in 2:16)
    {
      ...
      confusion <- confusionMatrix(pred1, normedTest[,13])$overall[1]
...
confusionMatrix(pred,normedTest[,13])

table(pred,normed[train_ind,13])

# Save everything 
# Save everything 
save(results, file=paste0("G:/Employee Folders/Dong Bing//ann.resultsSpecialSample2.RData")) 
save(models, file=paste0("G:/Employee Folders/Dong Bing//ann.modelsSpecialSample.RData")) 
save(best.ann, file=paste0("G:/Employee Folders/Dong Bing//best.annSpecialSample2.RData")) 
write.table(results, file=paste0("G:/Employee Folders/Dong Bing//ann.resultsSpecialSample2.csv"), sep=",")
