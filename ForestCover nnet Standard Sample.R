...
sampleIndex <- createDataPartition(normed$coverType, p = 0.1,list = FALSE,times = 1)
...
for (i in 1:1)
{
  for (c in 1:length(class.weights[,1]))
  {
    ...
    for (h in 2:16)
    {
      ...
      confusion <- confusionMatrix(pred[-train_ind], normed1[-train_ind,13])$overall[1]
      
      results <- rbind(results, c(h, tacc, wacc, confusion, c, 2))
...
best.row <- match(max(results[,3]), results[,3]) #Row 44
best.ann <- models[[best.row]]


#load("U:/R Training//best.ann.RData")

pred1 <- predict(best.ann, normed1[-train_ind,-13], type="class")

#best model

weighted.acc(pred1,normed1[-train_ind,13]) #0.7760712   



# Save everything 
save(results, file=paste0("G:/Employee Folders/Dong Bing//ann.resultsStandardSample2.RData")) 
save(models, file=paste0("G:/Employee Folders/Dong Bing//ann.modelsStandardSample.RData")) 
save(best.ann, file=paste0("G:/Employee Folders/Dong Bing//best.annStandardSample2.RData")) 
write.table(results, file=paste0("G:/Employee Folders/Dong Bing//ann.resultsStandardSample2.csv"), sep=",") 
