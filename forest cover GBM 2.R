...
gbmGrid <-  expand.grid(interaction.depth = c(5:30), #c(5:30), # 12 old; 25 was selected as optimal
                        n.trees = c(500:10000),# c(5000, 10000), #20000, 30000, 40000, 50000), 
                        shrinkage = c(0.01, 0.005), #c(0.01, 0.005),
                        n.minobsinnode = c(1, 3, 5, 7, 10)) #c(1, 3, 5, 7, 10))
...
load("G:/Employee Folders/Dong Bing//gbmFit.RData")
...
#Train Data Accuracy
weighted.acc(gbm_pred,data.train$coverType) #0.9432099 

confusionMatrix(gbm_pred, data.train$coverType) #0.9432
...
gbm_predTest <- predict(gbmFit, newdata = na.omit(data.test1), type="raw")

#Test Data Accuracy
weighted.acc(gbm_predTest,data.test1$coverType) #0.8423738

confusionMatrix(gbm_predTest, data.test1$coverType) #0.7106
